...
def calculate_demographic_data(print_data=True):
    ...
    country_salary_count = df[df["salary"] ==
                              '>50K']['native-country'].value_counts()
    country_count = df['native-country'].value_counts()
    highest_earning_country_table = country_salary_count / country_count
    ...
    highest_earning_country_percentage = round(
        (highest_earning_country_table.max() * 100), 1)

    # Identify the most popular occupation for those who earn >50K in India.
    df2 = df[(df["salary"] == ">50K")
             & (df["native-country"] == "India")].groupby(
                 "occupation", as_index=False).size()
    top_IN_occupation = df2.loc[df2["size"] == df2["size"].max(),
                                "occupation"].iloc[0]
